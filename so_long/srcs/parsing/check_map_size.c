...
/*   Updated: 2025/11/25 23:07:53 by caaubert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/so_long.h"

static bool check_map_size(char **map, t_map_size *map_size)
{
	int	i;
	int len;
...
static bool check_wall(char **map, t_map_size map_size)
{
	int	y;
	int x;
	...
	while (map[y])
	{
		...
		while (map[y][x])
		{
			if (y == 0 || x == 0 || y == (map_size.y - 1) || x == (map_size.x - 1))
			{
				if (map[y][x] != '1')
					return (ft_putstr_fd("Error\ninvalid map border\n", 2), false);
			}
...
{
	if (!check_map_size(map, map_size))
		return (false);
	if (!check_map_data(map))
		return (false);
	if (!check_wall(map, *map_size))
		return (false);
	return (true);
}